...
The aggregate throughput achieved by V4VSockets is shown below -- we are able to reach more than half of the system's memory bandwidth (measured with stream @ 27GB/s) %\footnote[1]{We
...
\headerbox{Acknowledgments}{name=acks,column=0,span=2,above=bottom}{
\small
...
\headerbox{WiP}{name=wip,column=2,span=2,above=bottom}{
%We plan to:
\begin{itemize}
\compresslist
\item Strengthen our implementation to a more user-friendly approach,
\item Thoroughly examine the CPU utilization overheads imposed by V4VSockets, 
\item Polish the peer discovery framework to adaptively use V4VSockets over generic sockets. 
\end{itemize}
